...
// function App() {
//   // keep track of ai (we keep this to avoid UI changes)
//   const [aiReady, setAiReady] = useState(false);

//   const [isLoading, setIsLoading] = useState(false);
//   // handle uploaded file
//   const [uploadedFile, setUploadedFile] = useState(null);
//   // store analysis done by ai
//   const [analysis, setAnalysis] = useState(null);
//   // store the extracted text from the pdf
//   const [resumeText, setResumeText] = useState("");
//   const [presenceChecklist, setPresenceChecklist] = useState([]);

//   // keep the same UI behavior: enable upload as soon as app mounts
//   useEffect(() => {
//     setAiReady(true);
//   }, []);
...
//   // ----- analyze resume via our backend (Gemini) -----
//   const analyzeResume = async (text) => {
//     // keep your original prompt replacement exactly as you wrote
//     const prompt = constants.ANALYZE_RESUME_PROMPT.replace(
//       "{{DOCUMENT_TEXT}}",
//       text
//     );

//     // send to backend; backend enforces JSON + schema
//     const resp = await fetch("http://localhost:8787/analyze", {
//       method: "POST",
//       headers: { "Content-Type": "application/json" },
//       body: JSON.stringify({ text: prompt }), // send the prompt with embedded document
//     });
...
//     if (!json?.success) {
//       throw new Error(json?.error || "Unknown analysis error");
//     }
//     return json.data;
//   };
...
//             font-light bg-gradient-to-r
//            from-[#B8A9FF] via-[#9C8CFF] to-[#7A78FF]
...
//               </h3>
//               <p
//                 className="text-slate-400 mb-4 sm:mb-6
//                     text-sm sm:text-base"
//               >
//                 PDF FILES ONLY and get feedback
//               </p>
//               <input
//                 type="file"
//                 accept=".pdf"
...
//                   <div
//                     className="icon-container-xl bg-gradient-to-br from-blue-500/20
//                         to-cyan-500/20 border-blue-500/30"
//                   >
//                     <span className="text-3xl">📄</span>
//                   </div>
//                   <div>
//                     <h3 className="text-xl font-bold text-violet-400 mb-1">
//                       Analysis Completed
//                     </h3>
...
//               <div className="bg-violet-500/20 
//               icon-container-lg 
//               mx-auto mb-3
...
//                     <span className="text-violet-400 text-sm mt-0.5">▸
//                       </span> 
...
//                 </div>
//                 <h4 className="text-blue-300 text-sm font-semibold uppercase mb-3">
//                   Improvements
//                 </h4>
//                 <div className="space-y-2 text-left">
...
//                         <span className="text-violet-200 font-medium text-sm leading-relaxed">
//                           {item}
//                         </span>
//                       </div>
//                     ))}
...
//                       </strong>{" "}
//                       is a software used by companies to collect and check
//                       resumes during hiring. When you apply for a job online,
//                       your resume first goes to the ATS, not directly to HR. The
//                       system scans your resume to see if your skills and
//                       experience match the job requirements. If it finds the
//                       right keywords, your resume moves forward to the
//                       recruiter. If not, it gets rejected before any human reads
//                       it.
//                     </p>
//                   </div>
//                 </div>
...
//                 <div className="flex items-center gap-3 mb-3">
//                   <span className="text-violet-300 text-lg">🧑‍💻</span>
//                   <h3 className="text-lg font-semi-bold text-violet-300">
...
//                     💡 Consider incorporationg these keywords naturally into your
...
//this is a perfectly working code and the problem is automatically analysing resume .
...
//   // ----- analyze resume via our backend (Gemini or local logic) -----
//   // note: we now send both the resumeText and the jobDescription to the server
//   const analyzeResume = async (text) => {
//     // keep your original prompt replacement exactly as you wrote
//     const prompt = constants.ANALYZE_RESUME_PROMPT.replace(
//       "{{DOCUMENT_TEXT}}",
...
//     // send to backend; backend expects JSON with resumeText and jobDescription
//     const resp = await fetch("http://localhost:8787/analyze", {
//       method: "POST",
//       headers: { "Content-Type": "application/json" },
//       body: JSON.stringify({
//         text: prompt, // original prompt (for compatibility)
//         resumeText: text, // plain resume text so server can run keyword matching
...
//       throw new Error(json?.error || "Unknown analysis error");
//     }
//     // server returns 'analysis' object, use it directly
...
//         {/* NEW: Job Description input (optional):*/}
//         {!uploadedFile && (
...
//             {/* NEW: ATS Match Score block (renders if backend returned atsScore) */}
...
//                       <span
//                         key={i}
//                         className="keyword-tag bg-emerald-600/20 border border-emerald-500"
//                       >
//                         {k}
//                       </span>
//                     ))}
...
//                   <h3 className="text-lg font-bold text-red-300 mt-4">
//                     ⚠ Missing Keywords
//                   </h3>
//                   <div className="flex flex-wrap gap-2 mt-2">
...
//                       <span
//                         key={i}
//                         className="keyword-tag bg-red-600/20 border border-red-500"
//                       >
//                         {k}
//                       </span>
//                     ))}
...
//                         No missing keywords detected for the provided JD.
//                       </p>
//                     )}
//                   </div>
//                 </div>
          ...
          <div className="space-y-6 p-4 sm:px-8 lg:p-16">
            ...
            {/* <div className="score-card">
              <div className="text-center mb-6">
                ...
                <div className="relative">
                  <p className="text-4xl sm:text-3xl font-extrabold text-slate-100 drop-show-lg">
                    {analysis.overallScore || "0/10"}
                  </p>
                </div>
                <div className={`inline-flex items-center gap-2 mt-3 px-4 py-2 rounded-full ${
                  parseInt(analysis.overallScore) >= 8 ? "score-status-excellent" :
                  parseInt(analysis.overallScore) >= 6 ? "score-status-good" :
                  "score-status-improvement"
                }`}>
                  <span className="text-lg">
                    {parseInt(analysis.overallScore) >= 8 ? "⭐️⭐️⭐️" :
                    parseInt(analysis.overallScore) >= 6 ? "⭐️⭐️" : "📈"}
                  </span>
                  <span className="font-semibold text-lg text-slate-200">
                    {parseInt(analysis.overallScore) >= 8 ? "Excellent" :
                    parseInt(analysis.overallScore) >= 6 ? "Good" : "Need Improvement"}
                  </span>
                </div>
              </div>
            </div> */}
